...
namespace MadamRozikaData.Services
{
    public class FooterService
    {
        ...
        public FooterDto GetFooter()
        {
            ...
            {
                CategoryMenuListDto = GetMenu(),
                FooterGalleryListDto = GetFooterGallery(),
                FooterNewsListDto = GetFooterNews()
            };
        }
        ...
        public List<FooterNewsDto> GetFooterNews()
        {
            return
                (from news in _db.News.Where(p => p.Status == 1).OrderByDescending(p => p.PublishDate).Take(5).ToList()
                    join category in _db.Categories.Where(p => p.Status == 1) on news.CategoryId equals
                        category.CategoryId
        ...
        public List<FooterGalleryDto> GetFooterGallery()
        {
            return
                (from gallery in
                    _db.Galleries.Where(p => p.Status == 1).OrderByDescending(p => p.PublishDate).Take(5).ToList()
                    join category in _db.Categories.Where(p => p.Status == 1) on gallery.CategoryId equals
                        category.CategoryId
